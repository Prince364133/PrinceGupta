...
export async function POST(request) {
    try {
        ...
        // Seed profile
        const profileRef = await addDoc(collection(db, 'profile'), sampleData.profile);
        results.profile = profileRef.id;

        // Seed projects
        for (const project of sampleData.projects) {
            const projectRef = await addDoc(collection(db, 'projects'), project);
            results.projects.push(projectRef.id);
        }

        // Seed startups
        for (const startup of sampleData.startups) {
            const startupRef = await addDoc(collection(db, 'startups'), startup);
            results.startups.push(startupRef.id);
        }

        // Seed skills
        for (const skill of sampleData.skills) {
            const skillRef = await addDoc(collection(db, 'skills'), skill);
            results.skills.push(skillRef.id);
        }
